...
function test_type_stability_for_type(FT)
    ...
    ts_eq = PhaseEquil_ρeq.(param_set, ρ, e_int, q_tot, 15, FT(rtol_temperature))
    ...
    ts_Tp = PhaseEquil_pTq.(
        param_set,
        air_pressure.(param_set, ts_eq),
        air_temperature.(param_set, ts_eq),
        q_tot,
    )
    ts_ρp = PhaseEquil_ρpq.(
        param_set,
        air_density.(param_set, ts_eq),
        air_pressure.(param_set, ts_eq),
        q_tot,
    ...
    ts_θ_liq_ice_eq = PhaseEquil_ρθq.(
        param_set,
        ρ,
        θ_liq_ice,
        q_tot,
    ...
    ts_θ_liq_ice_eq_p = PhaseEquil_pθq.(
        param_set,
        p,
        θ_liq_ice,
        q_tot,
    ...
    for ts in all_ts
        ...
        @test typeof.(liquid_specific_humidity.(param_set, ts)) == typeof.(e_int)
        ...
        @test typeof.(supersaturation.(param_set, ts, Liquid())) == typeof.(e_int)
        ...
        @test typeof.(total_specific_enthalpy.(param_set, ts, e_tot)) == typeof.(e_int)
        @test typeof.(moist_static_energy.(param_set, ts, e_pot)) == typeof.(e_int)
        @test typeof.(getproperty.(PhasePartition.(param_set, ts), :tot)) == typeof.(e_int)
        @test typeof.(virtual_dry_static_energy.(param_set, ts, e_pot)) == typeof.(e_int)
    end
    
    # Test individual function with scalar arguments
    @test typeof(q_vap_from_RH_liquid(param_set, FT(100000), FT(275), FT(0.8))) == FT
end
