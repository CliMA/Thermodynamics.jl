...
The saturation vapor pressure over a mixture of liquid and ice, given
 - `param_set` an `AbstractParameterSet`, see the [`Thermodynamics`](@ref) for more details
 - `T` temperature
...
The computation uses a weighted mean of the latent heats of vaporization and
sublimation, and specific heat capacities, weighted by the liquid fraction.
"""
...
Bool indicating if condensate exists, i.e., q_c > 0.
"""
@inline has_condensate(q_c) = q_c > 0
...
@inline function saturation_excess(
    ...
    FT = eltype(param_set)
    p_vap_sat = phase_type <: PhaseNonEquil ?
        saturation_vapor_pressure(param_set, T, q.liq, q.ice) :
        saturation_vapor_pressure(param_set, T)
