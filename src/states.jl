...
function PhaseDry(param_set::APS, e_int::FT, ρ::FT) where {FT}
    T = air_temperature(param_set, e_int)
    p = air_pressure(param_set, T, ρ)
    return PhaseDry{FT, typeof(param_set)}(param_set, e_int, ρ, p)
end
...
function PhaseEquil_pθq(
    ...
    return PhaseEquil{FT, typeof(param_set)}(param_set, ρ, p, e_int, q.tot, T)
end
...
function PhaseNonEquil(
    ...
    p = air_pressure(param_set, T, ρ, q)
    return PhaseNonEquil{FT, typeof(param_set)}(param_set, e_int, ρ, p, q)
end
...
function PhaseNonEquil_ρTq(
    ...
    e_int = internal_energy(param_set, T, q_pt)
    p = air_pressure(param_set, T, ρ, q_pt)
    return PhaseNonEquil{FT, typeof(param_set)}(param_set, e_int, ρ, p, q_pt)
end
